...
from HW08_koral_kalathia import file_reader
...
class Repository:
    ...
    def student_grades_table_db(self, db_path) -> None:
        ...
        for row in db.execute(
                "SELECT s.Name,s.CWID,g.Course,g.Grade,i.Name from students as s JOIN grades as g on g.StudentCWID = "
                "s.CWID JOIN instructors as i on g.InstructorCWID = i.CWID order by s.Name ASC"):
            pt.add_row(row)
            lst2.append(row)
...
def student_summary() -> str:
    ...
    db.close()
    return render_template('student.html', title="Stevens Repository", table_title=" Stevens Institute of Technology",
                           students=data)
